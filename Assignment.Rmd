...
preformed at different levels of quality. A machine learning algorithm was used
...
The data contains way to many predictors (`r length(train)` in total) to produce accurate and simple models and
requires some trimming. The first trim is preformed with the near zero variance function
...
```{r crossvalid}
Valid <- createDataPartition(train$classe, p=.3)[[1]]
valid <- train[Valid,]
train <- train[-Valid,]
```
...
reduced to `r sum(var)`. The data sets are now reduced with the following code.
...
at the entire training data set, it will require a lot of time. The models include:\
        ...
        - Linear Discriminant\
        - Combined\
The randomForest function is used as it is more efficient that the train function. 
...
has the same level of accuracy as the random forest meaning it is the most heavily 
